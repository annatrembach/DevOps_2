    ...
    while(1)
    {
        ...
        if (pid == 0)
        {
            ...
            if (!strcmp(strHTTP_requestMethod, "GET"))
            {
                ...
                else if (!strcmp(strHTTP_requestPath, "/trigvalues"))
		{
		    auto t1 = std::chrono::high_resolution_clock::now();

		    std::vector<double> trigValues;
		    for (int i = 0; i < 1500000; i++) {
			double value = sin(i) + cos(i) + tan(i);
			trigValues.push_back(value);
		    }

		    for (int i = 0; i < 1000; i++) {
			sort(begin(trigValues), end(trigValues));
			reverse(begin(trigValues), end(trigValues));
		    }
		    ...
		    write(clientSocket, strResponse, strlen(strResponse));
		    write(clientSocket, strTimeEllapsed, strlen(strTimeEllapsed));
		    printf("\nExecution time for trig values: %s ms\n", strTimeEllapsed);
		}
